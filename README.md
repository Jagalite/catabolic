...
Requires Python 3.11 or later on macOS or Linux. GraphQL uses the pinned
`graphql-core` dependency; other runtime features use the standard library.
...
Create a directory for application state and an empty directory for generated
links. Source and output directories must already exist when bound. Keep state
outside both trees.
...
selectors, Plex/flat presets, and safe mapping ownership. Save a layout, preview and
